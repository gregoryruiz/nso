...
export function NsoD3GraphDirective() {
    ...
    function nsoD3GraphDirectiveLink(scope, iElement, iAttrs) {
        ...
        d3.json("readme-flare-imports.json", (error, classes) => {
            if (error) {
                classes = flareImports();
            }

            const root = d3.hierarchy(packageHierarchy(classes), (d) => d.children);
        ...
        function packageHierarchy(classes) {
            ...
            function find(name, data?) {
                let nodeFound = map[name];
                let i;
                if (!nodeFound) {
                    nodeFound = map[name] = data || { name, children: [] };
                    if (name.length) {
                        nodeFound.parent = find(name.substring(0, i = name.lastIndexOf(".")));
                        nodeFound.parent.children.push(nodeFound);
                        nodeFound.key = name.substring(i + 1);
                    }
                }
                return nodeFound;
            }
