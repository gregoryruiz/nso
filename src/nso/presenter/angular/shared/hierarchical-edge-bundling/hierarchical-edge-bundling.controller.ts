...
export class HierarchicalEdgeBundlingController implements IComponentController {
  ...
  // TODO(@douglasduteil): use a real type here
  public vertex: { edges: any[], nodes: any[] };

  //

  private rootSvgElement: Element;
  ...
  public initialize() {
    d3.select(this.rootSvgElement)
      .attr("height", this.diameter)
      .attr("width", this.diameter);

    d3.select(this.stageElement)
      .attr("transform", `translate(${this.radius}, ${this.radius})`);

    this.cluster = d3.cluster<INodeDatum>()
      .size([360, this.innerRadius]);

    this.line = d3.radialLine<d3.HierarchyPointNode<INodeDatum>>()
      .angle((d) => d.x / 180 * Math.PI)
      .curve(d3.curveBundle.beta(0.95))
      .radius((d) => d.y);
  }

  public linkChart(context, edges) {
    const selection = context.selection ? context.selection() : context;
    const path = selection.selectAll("path").data(edges);
  ...
  public nodeChart(context, nodes) {
    const selection = context.selection ? context.selection() : context;
    const text = selection.selectAll("text").data(nodes);
    const nodeTransform = (d) => `rotate(${d.x - 90})`
      + `translate(${d.y + 8}, 0)`
      + (d.x < 180 ? "" : "rotate(180)");
  ...
  public $onChanges(onChangesObj: IOnChangesObject) {
    ...
    const classes = MOCK;
    const formatedData = packageHierarchy(classes);
    ...
    const linkChart = this.linkChart.bind(this);
    const nodeChart = this.nodeChart.bind(this);
    d3.select(this.linkElement).call(linkChart, links);
    d3.select(this.nodeElement).call(nodeChart, nodes);
  }

  public $postLink() {
    const rootEl = this.$element[0];
    this.rootSvgElement = rootEl.querySelector("svg");
    this.stageElement = rootEl.querySelector(".stage");
    this.linkElement = rootEl.querySelector(".link-group");
    this.nodeElement = rootEl.querySelector(".node-group");
...
function packageImports(nodes: Array<d3.HierarchyNode<INodeDatum>>): InputData[] {
  const map = {};
  const imports = [];
